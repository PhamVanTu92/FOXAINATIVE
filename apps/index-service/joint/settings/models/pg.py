"""PostgreSQL database configuration models for Index Service."""
...
class PostgresSettings(BaseModel):
    ...
    - POSTGRES__HOST: Service-specific host (POSTGRES__INDEX_HOST or POSTGRES__HOST)
    - POSTGRES__DB: Service-specific database (POSTGRES__INDEX_DB or POSTGRES__DB)
    """
    ...
    def resolve_host(cls, v: Optional[str]) -> str:
        ...
        Priority: POSTGRES__INDEX_HOST > POSTGRES__HOST > provided value
        """
        if v is None:
            val = os.getenv('POSTGRES__INDEX_HOST') or os.getenv(
                'POSTGRES__HOST',
            )
            return val if val is not None else 'localhost'
        return v
    ...
    def resolve_db(cls, v: Optional[str]) -> str:
        ...
        Priority: POSTGRES__INDEX_DB > POSTGRES__DB > provided value
        """
        if v is None:
            val = os.getenv('POSTGRES__INDEX_DB') or os.getenv('POSTGRES__DB')
            return val if val is not None else 'index_db'
        return v
