    ...
    <div>
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-primary-50/60 group transition-colors">
        ...
        <div className="hidden group-hover:flex items-center gap-1">
          <button onClick={() => onEdit(node)} className="p-1.5 text-warning-500 hover:bg-warning-50 rounded-lg transition-colors">
            <Pencil size={13} />
          </button>
          <button onClick={() => onDelete(node)} className="p-1.5 text-danger-400 hover:bg-danger-50 rounded-lg transition-colors">
            <Trash2 size={13} />
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-dark-200">
          <h2 className="font-semibold text-dark-800">{isNew ? '+ Thêm phòng ban' : 'Chỉnh sửa phòng ban'}</h2>
          <button onClick={onClose} className="text-dark-400 hover:text-dark-600"><X size={18} /></button>
        </div>
        <form onSubmit={submit} className="p-6 space-y-4">
          {isNew && (
            <div>
              <label className="block text-sm font-medium text-dark-700 mb-1">Mã phòng ban <span className="text-danger-600">*</span></label>
          ...
          <div>
            <label className="block text-sm font-medium text-dark-700 mb-1">Tên phòng ban <span className="text-danger-600">*</span></label>
            <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="VD: Phòng Kế toán - Tài chính"
              className="w-full border border-dark-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          {isNew && (
            <div>
              <label className="block text-sm font-medium text-dark-700 mb-1">Phòng ban trực thuộc</label>
              <select value={parentId} onChange={(e) => setParentId(e.target.value)}
                className="w-full border border-dark-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white">
          ...
          <div>
            <label className="block text-sm font-medium text-dark-700 mb-1">Người phụ trách</label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm border border-dark-200 rounded-lg hover:bg-dark-50">Hủy</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6">
        ...
        <p className="text-sm text-dark-600 mb-4">Bạn có chắc muốn xóa phòng ban <strong>{node.name}</strong>? Hành động này không thể hoàn tác.</p>
        {error && <p className="text-sm text-danger-600 mb-3">{error}</p>}
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm border border-dark-200 rounded-lg hover:bg-dark-50">Hủy</button>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-6 py-4 border-b border-dark-200 bg-white">
        ...
        <span className="text-sm font-medium text-dark-700">Cơ cấu tổ chức</span>
      </div>

      {/* Title */}
      <div className="px-6 pt-5 pb-1 bg-white">
        <h1 className="text-xl font-semibold text-dark-800">Cơ cấu tổ chức</h1>
      </div>

      {/* Toolbar */}
      <div className="flex items-center gap-3 px-6 py-4 bg-white border-b border-dark-200">
        <div className="flex border border-dark-200 rounded-lg overflow-hidden">
          <button onClick={() => setView('table')}
            className={`flex items-center gap-1.5 px-3 py-1.5 text-sm ${view === 'table' ? 'bg-primary-600 text-white' : 'text-dark-600 hover:bg-dark-50'}`}>
            <LayoutList size={14} /> Bảng
          </button>
          <button onClick={() => setView('tree')}
            className={`flex items-center gap-1.5 px-3 py-1.5 text-sm border-l border-dark-200 ${view === 'tree' ? 'bg-primary-600 text-white' : 'text-dark-600 hover:bg-dark-50'}`}>
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm kiếm phòng ban..."
            className="w-full pl-9 pr-3 py-1.5 text-sm border border-dark-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" />
        </div>

        <div className="flex-1" />

        <button onClick={exportExcel}
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-dark-200 rounded-lg hover:bg-dark-50 text-dark-600">
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {loading ? (
          <div className="flex items-center justify-center h-40 text-dark-400 text-sm">Đang tải...</div>
        ) : view === 'tree' ? (
          <div className="bg-white rounded-xl border border-dark-200 shadow-sm p-4">
            {tree.length === 0 ? (
              <p className="text-sm text-dark-400 text-center py-10">Chưa có phòng ban nào.</p>
            ) : (
              tree.map((n) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-dark-50 border-b border-dark-200">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wide">Mã phòng ban</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wide">Tên phòng ban</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wide">Người phụ trách</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wide">Phòng ban trực thuộc</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-dark-500 uppercase tracking-wide">Thao tác</th>
                </tr>
              </thead>
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  paginated.map((node, idx) => (
                    <tr key={node.id} className="border-b border-dark-100 last:border-0 hover:bg-dark-50 transition-colors">
                      <td className="px-4 py-3"><input type="checkbox" className="rounded" /></td>
                      <td className="px-2 py-3 text-dark-500">{(page - 1) * pageSize + idx + 1}</td>
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 bg-warning-100 text-warning-700 font-mono text-xs px-2 py-0.5 rounded">
                          #{node.code}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-medium text-dark-800">{node.name}</td>
                      <td className="px-4 py-3">
                        {node.managerName ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-dark-700">{node.managerName}</span>
                          </div>
                      ...
                      <td className="px-4 py-3">
                        {node.parentName
                          ...
                          : <span className="text-dark-400 italic text-xs">— Cấp cao nhất —</span>}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setEditingNode(node)} className="p-1.5 text-warning-500 hover:bg-warning-50 rounded-lg" title="Chỉnh sửa">
                            <Pencil size={15} />
                          </button>
                          <button onClick={() => setDeletingNode(node)} className="p-1.5 text-danger-400 hover:bg-danger-50 rounded-lg" title="Xóa">
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </td>
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-dark-100 text-sm text-dark-500">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                    className="px-2 py-1 border border-dark-200 rounded hover:bg-dark-50 disabled:opacity-40">&lsaquo;</button>
                  ...
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                    className="px-2 py-1 border border-dark-200 rounded hover:bg-dark-50 disabled:opacity-40">&rsaquo;</button>
                </div>
              </div>
            )}
          </div>
