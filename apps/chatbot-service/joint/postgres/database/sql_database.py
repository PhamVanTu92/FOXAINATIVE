"""SQL database connection manager for Query Service.

Provides both sync (psycopg2) and async (psycopg v3) engines so that:
- Sync engine  → used by Alembic migrations and legacy code paths.
- Async engine → used by hot-path domain services in the async event loop.
"""
...
class SQLDatabase:
    ...
    def __init__(
        ...
        *,
        ...
        # Engines stored explicitly for reliable lifecycle management.
    ...
    def sessionmaker(self) -> sessionmaker:
        ...
        Pool configuration comes from constructor args which are typically
        sourced from ``PostgresSettings`` (env-driven).
        """
    ...
    def dispose_sync(self) -> None:
        ...
        Safe to call even if the engine was never created (no-op).
        """
