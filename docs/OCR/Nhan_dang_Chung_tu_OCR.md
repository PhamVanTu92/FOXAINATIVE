...
> **Async Queue:** `ocr-queue` (đơn lẻ) / `file-analysis-queue` (file lớn, bulk) – Redis + BullMQ
...
> **Xử lý bất đồng bộ (Async Layer):**
> - Upload **1 file đơn:** OCR Service xử lý đồng bộ trong request scope → trả kết quả ngay.
> - Upload **nhiều file (Bulk OCR)** hoặc **file lớn (PDF > 10 trang):** OCR Service **dispatch job** vào BullMQ queue, trả về `jobId` ngay lập tức. Frontend polling `jobId` để lấy trạng thái và kết quả.
>   - File thông thường → `ocr-queue` (OCR Service Worker tiêu thụ).
>   - File lớn / phân tích phức tạp → `file-analysis-queue` (OCR Service Worker tiêu thụ).
> - **Retry policy:** 3 lần, exponential backoff. Job payload chỉ chứa file reference ID, không chứa binary.
...
> **Lưu ý:** Toàn bộ pipeline diễn ra **ngầm trong OCR Service Worker** (consumer của BullMQ queue), người dùng không cần biết hoặc tương tác. Mục tiêu duy nhất: nâng accuracy của OCR Engine lên trên 95%.
...
> 📌 **Tài liệu liên quan:**
> - [[OCR/Thiet_lap_Chung_tu_OCR]] – Cung cấp schema cho phân hệ này hoạt động.
> - [[OCR/Quan_ly_Chung_tu_OCR]] – Nơi tiếp nhận chứng từ sau khi xác nhận.
> - [[OCR/Database_Design_OCR_System]] – Thiết kế bảng `documents`, `document_values`, `document_line_items`.
> - [[Tech_Stack_Architecture]] – Kiến trúc tổng thể: OCR Service, BullMQ queues, gRPC.
